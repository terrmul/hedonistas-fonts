...
function initMapGate(root){
  root=root||document;
  var gate=root.querySelector('#map-gate');
  var section=root.querySelector('#findus')||(root.id==='findus'?root:null);
  if(!gate||!section||gate.dataset.mg) return;
  gate.dataset.mg='1';
  var timer=null;
  gate.addEventListener('click',function(){gate.classList.add('active')});
  gate.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' ')gate.classList.add('active');});
  section.addEventListener('mouseleave',function(){timer=setTimeout(function(){gate.classList.remove('active')},3000)});
...
var SECTION_INITS={
  ...
  'findus':   function(el){ initMapGate(el); },
